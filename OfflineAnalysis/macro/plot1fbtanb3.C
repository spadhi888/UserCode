...
//---LEP curves
  double ch_m0[] = {0,0,600,1200,1200};
  double ch_m12[] = {0,160,140,120,0};
...
int nss0 = 21;
 double xss0[nss0]={0,50,100,150,200,250,300,350,400,450,500,550,600,650,700,750,800,850,900,950,1000};
 double yss0[nss0]={340.00,360.00,300.00,280.00,240.00,240.00,260.00,260.00,280.00,280.00,280.00,220.00,200.00,200.00,200.00,160.00,160.00,140.00,120.00,140.00,140.00};
...
//gStyle->SetHatchesLineWidth(2);
 ...
 vC1->cd(1);
 TH1F *vFrame = gPad->DrawFrame(0,100,1000,500);
 ...
 //g_cdf->Draw("P");
 //g_cdf1->Draw("P");
 // TText *t1 = new TLatex(100,360,"CMS Preliminary @ 7 TeV, 1 fb^{-1} ");
 // t1->SetTextSize(0.028);
 // t1->Draw();
 
 TLegend *leg = new TLegend(0.4,0.7, 0.9, 0.97, "CMS Preliminary, \\sqrt{s} = 7 TeV, 1 fb^{-1}");
 leg.SetFillStyle(0);
 leg.SetBorderSize(0);
 leg.SetTextSize(0.03);

 leg->AddEntry(g_ss0,"Observe 4 event","l");
 leg->AddEntry(cdfn1,"CDF 3l 95% CL (3.2 fb^{-1})","f");
 leg->AddEntry(ch_gr,"LEP excluded regions","f");
 leg->Draw();
// vC1->SaveAs("exclusion1fbss.eps");
 vC1->SaveAs("exclusion1fbss.png");
