...
// $Revision: 1.19 $
//-----------------------------------------------------------------------------
#include "pmssmanalyzerssleptons.h"
...
//  double xsect = 38.93; // LM0
  ...
  for (int entry=0; entry < nevents; ++entry)
	...
	// Hypothesis
	pair<float,int> lep(Electron[i].PT, (Electron[i].Charge)*11);
	hypothese.push_back(lep);
	...
	// Hypothesis
	pair<float,int> lep(Muon[i].PT, (Muon[i].Charge)*13);
	hypothese.push_back(lep);	
