...
public: 
  ...
  //function called if one only wants to use the BB events, or if one wants to use all events but with the single electron gun weights
  int ScanChain_BB( TChain* chain, int nEventsMax = -1, std::string region="bb", 
		    bool useSingleElectronWeights = false);
  //function called when running over BE events
  int ScanChain_BE( TChain* chain, int nEvents = -1, std::string BBfname="");
