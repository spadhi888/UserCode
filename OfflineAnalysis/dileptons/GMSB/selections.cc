...
bool isGoodLeptonLooseID(int id, int lepIdx) {
  ...
  if(abs(id) == 11) {
    if (!pass_electronSelection(lepIdx, electronSelection_ttbarV1_noiso)) return false;
  }
...
bool isGoodLeptonwIso(int id, int lepIdx) { //, bool used0wrtPV) {
  ...
  // 11 is a electron
  if(abs(id)== 11) {
    if (!pass_electronSelection(lepIdx, electronSelection_ttbarV1_iso)) return false;
  }
