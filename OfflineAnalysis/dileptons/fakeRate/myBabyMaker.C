...
<<<<<<< myBabyMaker.C
 //   set_goodrun_file_json("json/Cert_TopAug13_Merged_135059-142664.txt");
    set_goodrun_file_json("json/json_133446_144114_3.1pb.txt");
=======
...
/* Electrons */
      ...
      for (unsigned int iEl = 0 ; iEl < els_p4().size(); iEl++) {
        ...
        v4SS_  = pass_electronSelection(iEl, electronSelectionFO_ssVBTF80_v4);
        ...
        v4SSAug9_  = pass_electronSelection(iEl, electronSelectionFO_ssVBTF80_v4, isData, true);
	...
	v3Oct6_  = pass_electronSelection( iEl, electronSelectionFO_el_ttbarV1_v3);
        ...
        if (numSS_ && (!v4SS_)) cout << "bad v4" << endl;
        ...
        if (  (!v1_) && (!v2_) && (!v3_) && 
              (!v1SS_) && (!v2SS_) && (!v3SS_) && (!v4SS_) &&
              (!v1Aug9_) && (!v2Aug9_) && (!v3Aug9_) &&
<<<<<<< myBabyMaker.C
              (!v1SSAug9_) && (!v2SSAug9_) && (!v3SSAug9_)  && (!v4SSAug9_)
=======
...
//     PF Jets
        nbpfjet_ = this_nbpfjet;
        dRbpfNear_ = 99.;
        dRbpfFar_  = -99.;
        for (int ii=0; ii<nbpfjet_; ii++) {
          unsigned int iJet = bpfindex[ii];
          LorentzVector jp4 = pfjets_p4()[iJet];
          float jet_cor = jetCorrection(jp4, jet_corrector);
          LorentzVector jp4cor = jp4 * jet_cor;
          float dr = ROOT::Math::VectorUtil::DeltaR( els_p4().at(iEl), jp4cor);
          if (dr < dRbpfNear_) dRbpfNear_ = dr;
          if (dr > dRbpfFar_)   dRbpfFar_  = dr; 
        }
        ...
        for (unsigned int iJet = 0; iJet < jets_p4().size(); iJet++) {
         if (jets_p4().at(iJet).pt() > 15. && jets_simpleSecondaryVertexHighEffBJetTag().at(iJet) > 1.74) btag_ = true;
...
<<<<<<< myBabyMaker.C
       // Find the highest Pt PF jet separated by at least dRcut from this lepton and fill the jet Pt
       ...
       // JetID
          ...
          float jet_cor = jetCorrection(jp4, jet_corrector);
          LorentzVector jp4cor = jp4 * jet_cor;
          if (jp4cor.pt() > 15 && pfjets_simpleSecondaryVertexHighEffBJetTag().at(iJet) > 1.74 ) btagpf_ = true;
          double dr = ROOT::Math::VectorUtil::DeltaR( els_p4().at(iEl), jp4cor );
          if( dr > deltaRCut && jp4cor.pt() > 10 ) npfj1_++;
          if ( dr > deltaRCut && jp4cor.pt() > ptpfj1_ ){
            ptpfj1_ = jp4cor.pt();

            // back to back in phi
            float dphi = fabs( ROOT::Math::VectorUtil::DeltaPhi( els_p4().at(iEl), jp4cor ) );
            if( dphi > deltaPhiCut && jp4cor.pt() > ptpfj1_b2b_ ){
              ptpfj1_b2b_   = jp4cor.pt();
              dphipfj1_b2b_ = dphi;
            } 
          }
        }
...
void myBabyMaker::MakeBabyNtuple(const char *babyFilename)
{
    ...
    babyTree_->Branch("v4SS",         &v4SS_,        "v4SS/O"      );
    ...
    babyTree_->Branch("v4SSAug9",         &v4SSAug9_,        "v4SSAug9/O"      );
...
<<<<<<< myBabyMaker.C
    babyTree_->Branch("btag",       &btag_,       "btag/O"      );


    babyTree_->Branch("nbpfjet",       &nbpfjet_,       "nbpfjet/I"      );
    babyTree_->Branch("dRpfNear",       &dRbpfNear_,       "dRbpfNear/F"      );
    babyTree_->Branch("dRpfFar",       &dRbpfFar_,       "dRbpfFar/F"      );
    ...
    babyTree_->Branch("btagpf",       &btagpf_,       "btagpf/O"      );
=======
    babyTree_->Branch("ptpfj1",       &ptpfj1_,       "ptpfj1/F"      );
    babyTree_->Branch("npfj1",       &npfj1_,       "npfj1/I"      );
    babyTree_->Branch("ptpfj1_b2b",       &ptpfj1_b2b_,       "ptpfj1_b2b/F"      );
