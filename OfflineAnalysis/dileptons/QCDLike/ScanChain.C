...
// deal with MC later
	...
	if (isatty(1)) {
      ...
      if ((evt_lumiBlock() == 897) && (evt_event() == 798059318)) {
        cout << "after run,lumi,number: " << evt_run() << "," << evt_lumiBlock() << "," << evt_event() << endl;
      }
...
/*
        if(abs(id_lt) == 13 && abs(id_ll) == 13 && (evt_lumiBlock() == 897) && (evt_event() == 798059318))) {
            cout << muonIsoValue(idx_ll) << "  " << muonIsoValue(idx_lt) << endl;
         }
*/	
