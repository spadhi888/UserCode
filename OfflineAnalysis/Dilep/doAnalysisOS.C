...
TH1F* hnSumEt[4];       // Njet distributions
TH1F* hnLeadingEt[4];       // Njet distributions
...
//     if (mischarge) return;
 ...
 //    if (njets < 1) return;
     ...
     hnLeadingEt[myType]->Fill(calo_jetsp4[0].pt(), weight);
     hnLeadingEt[3]->Fill(calo_jetsp4[0].pt(), weight);

     hnSumEt[myType]->Fill(sumet_calo, weight);
     hnSumEt[3]->Fill(sumet_calo, weight);
...
RooDataSet* ScanChain( TChain* chain, enum Sample sample, bool identifyEvents ) {
 ...
 // declare and create array of histograms
  ...
  for (int i=0; i<4; i++) {
    ...
    hnLeadingEt[i] = new TH1F(Form("%s_hnLeadingEt_%s",prefix,suffix[i]),Form("%s_hnLeadingEt_%s",prefix,suffix[i]),200,0.,2000.);
    hnSumEt[i] = new TH1F(Form("%s_hnSumEt_%s",prefix,suffix[i]),Form("%s_hnSumEt_%s",prefix,suffix[i]),200,0.,2000.);
