{
 // Makes basic histograms from the root tree
  ...
  empty->GetYaxis()->SetTitle("m(#tilde{b}) GeV");
  ...
  char * selection ="Same Sign with btag selection  m(#chi^{0}) = 50 GeV";
  char * masses = Form("m(#chi^{0}) = 50 GeV");
  ...
  gg2.DrawLatex(xmin+0.05*(xmax-xmin), ymax-0.20*(ymax-ymin), "Exclusion #sigma^{prod} = #sigma^{NLO+NLL} #pm 1 #sigma");

  latexLabel.DrawLatex(500, 340, "#color[2]{m(#chi^{+}) = 150 GeV}");
  latexLabel.DrawLatex(560, 485, "#color[4]{m(#chi^{+}) = 300 GeV}");
