{
  ...
  excl->SetTitle("Gluino Sbottom model  Excluded points in red");
  ...
  acc->SetTitle("Gluino Sbottom model  Acc*Eff*BR for best signal region in percent");
  ...
  char * masses = Form("m(#chi^{0}) = 50 GeV and m(#chi^{+}) = %i GeV",(int) mchi);
  ...
  l1.SetLineColor(1);
  l1.SetLineWidth(3);
  TLine l2 = TLine(xmin+0.05*(xmax-xmin), ymax-0.275*(ymax-ymin), xmin+0.14*(xmax-xmin), 
                   ymax-0.275*(ymax-ymin));
  l2.SetLineColor(1);
  l2.SetLineWidth(3);
  l2.SetLineStyle(2);
