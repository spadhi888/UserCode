{
  ...
  excl->SetTitle("Gluino stop model  Excluded points in red");
  ul->GetYaxis()->SetTitle(lspmass);
  ul->GetXaxis()->SetTitle(glmass);
  ul->SetTitle("Gluino stop model  Cross-section upper limits (pb)");
  empty->GetYaxis()->SetTitle(lspmass);
  empty->GetXaxis()->SetTitle(glmass);
  empty->SetTitle("Gluino stop model");
  ulbest->GetYaxis()->SetTitle(lspmass);
  ulbest->GetXaxis()->SetTitle(glmass);
  ulbest->SetTitle("Gluino stop  Best region based on exp. limit");
  acc->GetYaxis()->SetTitle(lspmass);
  acc->GetXaxis()->SetTitle(glmass);
  acc->SetTitle("Gluino stop model  Acc*Eff*BR for best signal region in percent");
  ...
  TLine l1 = TLine(xmin+0.05*(xmax-xmin), ymax-0.225*(ymax-ymin), xmin+0.14*(xmax-xmin), 
                   ymax-0.225*(ymax-ymin));
  l1.SetLineColor(4);
  l1.SetLineWidth(3);
  TLine l2 = TLine(xmin+0.05*(xmax-xmin), ymax-0.250*(ymax-ymin), xmin+0.14*(xmax-xmin), 
                   ymax-0.250*(ymax-ymin));
  l2.SetLineColor(4);
  l2.SetLineWidth(3);
  l2.SetLineStyle(2);
  ...
  latexLabel.DrawLatex(xmin+0.05*(xmax-xmin), ymax-0.20*(ymax-ymin),"#sqrt{s} = 7 TeV L=4.7 fb^{-1} m(#tilde{#chi^{0}}) = 50 GeV");
