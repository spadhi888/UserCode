...
// of time, eg,
// sort sbottomtop_mlsp50_sr1.txt > SR1_50_sorted.txt
// sort sbottomtop_mlsp50_sr2.txt > SR2_50_sorted.txt
// sort sbottomtop_mlsp50_sr3.txt > SR3_50_sorted.txt
// sort sbottomtop_mlsp50_sr4.txt > SR4_50_sorted.txt
// sort sbottomtop_mlsp50_sr5.txt > SR5_50_sorted.txt
// sort sbottomtop_mlsp50_sr6.txt > SR6_50_sorted.txt
// sort sbottomtop_mlsp50_sr7.txt > SR7_50_sorted.txt
//
//
// Also reads in the sbottom xsection txt file.  
// Assumes that the sbottom mass increases monotonycally
...
void makeNtuple () {
    ...
    //-----------------------------------------------
    // Raed in the sbottom pair production xsection  
    //-----------------------------------------------
    ...
    float xsecdwn[2000];
    int nxsec=0;
    ifstream fromfile("../../xsec/sbottom_xsec.txt");
    if ( !fromfile.good() ) {
        cout << "sbottom xsec file does not exist" << endl ;
        return;
    }
    cout << "Reading sbottom xsection" << endl;
    int xs=0;
    while (!fromfile.eof()) {
        TString d1, d2, d3, d4;
    ...
    //--------------------------------------------------------
    // Check that the sbottom mass in the xsection file increases monotonically
    //---------------------------------------------------------
    ...
    filenames.push_back("../txt/sbottomtop_mlsp50_sr1.txt");
    filenames.push_back("../txt/sbottomtop_mlsp50_sr2.txt");
    filenames.push_back("../txt/sbottomtop_mlsp50_sr3.txt");
    filenames.push_back("../txt/sbottomtop_mlsp50_sr4.txt");
    filenames.push_back("../txt/sbottomtop_mlsp50_sr5.txt");
    filenames.push_back("../txt/sbottomtop_mlsp50_sr6.txt");
    // filenames.push_back("../txt/sbottomtop_mlsp50_sr7.txt");

    //-----------------------------------------------
    // Loop over signal regions and load the arrays from the txt files
    //-----------------------------------------------
    for (int ireg=0; ireg<filenames.size(); ireg++) {
        nentries[ireg] = 0;
    ...
    for (int ii=0; ii<filenames.size(); ii++) {
        for (int jj=ii+1; jj<filenames.size(); jj++) {
