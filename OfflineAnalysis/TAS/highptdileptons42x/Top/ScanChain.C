...
void FillHistograms(const unsigned int hypIdx, const vector<LorentzVector> v_jetsp4, const vector<unsigned int> v_jets, 
                    const vector<unsigned int> v_jetsNoEtaCut, const pair<float, float>, const float weight, std::string prefix);
...
void ScanChain( TChain* chain, vector<TString> v_Cuts, string prefix="", 
  ...
  if(scaleJESMETDown && scaleJESMETUp) {
    cout << "You have chosen to scale the JES up and down at the same time. Exiting" << endl;
    return;
  }
...
// JEC 
  ...
  std::vector<std::string> jetcorr_resfilenames;
  if (usecaloJets)
  {
         ...
         jetcorr_resfilenames.push_back("../CondFormats/JetMETObjects/data/Spring10DataV2_L2L3Residual_AK5Calo.txt");
  } 
  else if (usejptJets)
  {
         ...
         jetcorr_resfilenames.push_back("../CondFormats/JetMETObjects/data/Spring10DataV2_L2L3Residual_AK5JPT.txt");
  }
  else if (usepfJets)
  {
        ...
        jetcorr_resfilenames.push_back("../CondFormats/JetMETObjects/data/Spring10DataV2_L2L3Residual_AK5PF.txt");
  }

  FactorizedJetCorrector *jet_corrector = makeJetCorrector(jetcorr_filenames);
  FactorizedJetCorrector *jet_rescorrector = makeJetCorrector(jetcorr_resfilenames);
...
// Jet energy scale
  if (scaleJESMETUp) {
    if(usecaloJets) 
      globalJESRescale = 1.1;
    else
      globalJESRescale = 1.05;
  }
  if (scaleJESMETDown) {
    if(usecaloJets) 
      globalJESRescale =  0.90;
    else 
      globalJESRescale =  0.95;
  }
...
//      if (!cleaning_standardAugust2010()) continue;
      ...
      int ntaus = 0;
      int nleps = 0;
      if(prefix != "data") nleps = leptonGenpCount_lepTauDecays(nels, nmus, ntaus);
...
//      if (prefix == "DYtautau" &&  ntaus != 2) continue;
      if (prefix != "data") nleps = leptonGenpCount(nels, nmus, ntaus);
      if (prefix == "DYtautau" &&  ntaus != 2) continue;
	...
	if(excludePtGt2020) {
	  ...
	  if(lt_p4.Pt() < 10 || ll_p4.Pt() < 10.) continue; 
        ...
        if(applyTriggers) {
         ...
         //MuMu trigger fiduciality cut
          if(type == 0) {
            if(fabs(lt_p4.Eta()) > 2.1 && fabs(ll_p4.Eta()) > 2.1) continue;
          }
          if(type == 1 || type == 2) {
            if((abs(id_lt) == 11 && els_eSC()[idx_lt]/cosh(els_etaSC()[idx_lt]) < 17) && 
               (abs(id_ll) == 13 && fabs(ll_p4.Eta()) > 2.1)) continue;
            if((abs(id_ll) == 11 && els_eSC()[idx_ll]/cosh(els_etaSC()[idx_ll]) < 17) && 
               (abs(id_lt) == 13 && fabs(lt_p4.Eta()) > 2.1)) continue;
          }
        }//applyTriggers
...
// Require Z mass in ee only
	...
	if(requireZmass) {
	  if(type == 0 || type == 3) {
        ...
        if(hyp_p4()[hypIdx].mass() < 12.) continue;
	...
	if(applylepIsoCuts) {
     ...
     if (chargeFlip) { // check it
       ...
       if (useFlipRateEstimation) {  // check it
	...
	if(usecaloJets) {
	  for(unsigned int i = 0; i < jets_p4().size(); i++) {
            ...
            jet_cor = jetCorrection(jp4, jet_corrector);
/*
            if(prefix == "data") 
              jet_cor = jetCorrection(jp4, jet_rescorrector);
             else 
              jet_cor = jetCorrection(jp4, jet_corrector);
*/
            v_jetP4s.push_back(jp4 * jet_cor * globalJESRescale);
           }
	}
	if(usejptJets) {
...
// BTag jets
        ...
        if(requireBTag) {
          pair<int, float> p_btag;
          if (requireTCHEL) {
            if(usecaloJets) p_btag = getMinbltags(hypIdx, v_jetP4s, v_goodJets, "caloJets", "trackCountingHighEffBJetTag");
            if(usejptJets)  p_btag = getMinbltags(hypIdx, v_jetP4s, v_goodJets, "jptJets", "trackCountingHighEffBJetTag");
            if(usepfJets)   p_btag = getMinbltags(hypIdx, v_jetP4s, v_goodJets, "pfJets", "trackCountingHighEffBJetTag");
          } else {
            if(usecaloJets) p_btag = getMinbltags(hypIdx, v_jetP4s, v_goodJets, "caloJets", "simpleSecondaryVertexHighEffBJetTag");
            if(usejptJets)  p_btag = getMinbltags(hypIdx, v_jetP4s, v_goodJets, "jptJets", "simpleSecondaryVertexHighEffBJetTag");
            if(usepfJets)   p_btag = getMinbltags(hypIdx, v_jetP4s, v_goodJets, "pfJets", "simpleSecondaryVertexHighEffBJetTag");
          }
          if (p_btag.first < 1) continue;
//          if (p_btag.second < 160) continue;
        }
	...
	//in the nJet > 1 bin
	if(usePtGt2010) {
	  if(max(hyp_lt_p4()[hypIdx].Pt(), hyp_ll_p4()[hypIdx].Pt()) < 20.) continue;
	  if(min(hyp_lt_p4()[hypIdx].Pt(), hyp_ll_p4()[hypIdx].Pt()) < 10.) continue;
	}//if(usePtGt2010)
...
*/
	...
	FillHistograms(hypIdx, v_jetP4s, v_goodJets, v_goodJetsNoEtaCut, p_met, weight, prefix);	
...
//    std::cout << "ERROR: number of events from files is not equal to total number of events" << std::endl;
  ...
  delete jet_rescorrector; 


}

void FillHistograms(const unsigned int hypIdx, const vector<LorentzVector> v_jetsp4, const vector<unsigned int> v_jets, const vector<unsigned int> v_jetsNoEtaCut,
  ...
  for(unsigned int i = 0; i < v_jets.size(); i++) {
    v_jets_p4.push_back(v_jetsp4[v_jets.at(i)]);
  }
  
  //for MT2
  vector<LorentzVector> v_jetsNoEtaCut_p4;
  for(unsigned int i = 0; i < v_jetsNoEtaCut.size(); i++) {
     v_jetsNoEtaCut_p4.push_back(v_jetsp4[v_jetsNoEtaCut.at(i)]);
  }
  ...
  for(unsigned int i = 0; i < v_jets.size(); i++) {
    if (requireTCHEL) { 
     if (usecaloJets && jets_trackCountingHighEffBJetTag()[v_jets[i]] > 1.7) v_bjets_p4.push_back(v_jetsp4[v_jets.at(i)]);
     if (usejptJets && jpts_trackCountingHighEffBJetTag()[v_jets[i]] > 1.7) v_bjets_p4.push_back(v_jetsp4[v_jets.at(i)]);
     if (usepfJets && pfjets_trackCountingHighEffBJetTag()[v_jets[i]] > 1.7) v_bjets_p4.push_back(v_jetsp4[v_jets.at(i)]);    
    } else {
     if (usecaloJets && jets_simpleSecondaryVertexHighEffBJetTag()[v_jets[i]] > 1.74) v_bjets_p4.push_back(v_jetsp4[v_jets.at(i)]);
     if (usejptJets && jpts_simpleSecondaryVertexHighEffBJetTag()[v_jets[i]] > 1.74) v_bjets_p4.push_back(v_jetsp4[v_jets.at(i)]);
     if (usepfJets && pfjets_simpleSecondaryVertexHighEffBJetTag()[v_jets[i]] > 1.74) v_bjets_p4.push_back(v_jetsp4[v_jets.at(i)]);
    }
  }

    pair<int, float> p_btags;
    if (requireTCHEL) {
       if(usecaloJets) p_btags = getMinbltags(hypIdx, v_jetsp4, v_jets, "caloJets", "trackCountingHighEffBJetTag");
       if(usejptJets)  p_btags = getMinbltags(hypIdx, v_jetsp4, v_jets, "jptJets", "trackCountingHighEffBJetTag");
       if(usepfJets)   p_btags = getMinbltags(hypIdx, v_jetsp4, v_jets, "pfJets", "trackCountingHighEffBJetTag");
    } else {
       if(usecaloJets) p_btags = getMinbltags(hypIdx, v_jetsp4, v_jets, "caloJets", "simpleSecondaryVertexHighEffBJetTag");
       if(usejptJets)  p_btags = getMinbltags(hypIdx, v_jetsp4, v_jets, "jptJets", "simpleSecondaryVertexHighEffBJetTag");
       if(usepfJets)   p_btags = getMinbltags(hypIdx, v_jetsp4, v_jets, "pfJets", "simpleSecondaryVertexHighEffBJetTag");
    }
...
//     cout << "MAsses " << min(llbjet.mass(), ltbjet.mass()) << "  " << max(llbjet.mass(), ltbjet.mass()) << endl;
  } else if (v_bjets_p4.size() > 1) {
      bbjet = v_bjets_p4[0] + v_bjets_p4[1];
  }
//  cout << "MAss " << v_bjets_p4[0].mass() << "  " << v_bjets_p4[1].mass() << " " << v_bjets_p4[0].E()* v_bjets_p4[1].E() << " " << v_bjets_p4[0].px()*v_bjets_p4[1].px() + v_bjets_p4[0].py()*v_bjets_p4[1].py() + v_bjets_p4[0].pz()*v_bjets_p4[1].pz()<< endl;

//  cout << "Ntags " << v_bjets_p4.size() << " notags " << v_jets_p4.size() << endl;
  ...
  unsigned int bjetBin = p_btags.first;
  if(jetBin > 3) jetBin = 4;
  if(bjetBin > 3) bjetBin = 4;
  ...
  for(unsigned int i = 0; i < v_type.size(); i++) {
    ...
    hnbJet[ch]                         ->Fill(bjetBin,        weight);
    hnblJetMin[ch]                         ->Fill(p_btags.second,        weight);
    hnblJetMax[ch]                         ->Fill(max(llbjet.mass(), ltbjet.mass()),        weight);
    hnbbJet[ch]                            ->Fill(bbjet.mass(),        weight);
    if (p_btags.first > 1) hmt2BJ[ch]      ->Fill(TMath::Min(double(MT2J(p_met.first, p_met.second, lt_p4, ll_p4, v_bjets_p4, 0.0, BISECT, false)), 300.),weight);
    if (tttype == 1) {
...
//-s      float relIso = electronIsolation_relsusy_cand1(ll_idx, true);
    ...
    hmet[ch][jetBin]                  ->Fill(TMath::Min(double(p_met.first), 100.),    weight); 
    hmetPhi[ch][jetBin]               ->Fill(p_met.second, weight); 
    hpfmet[ch][jetBin]                ->Fill(TMath::Min(double(evt_pfmet()), 100.),             weight);
...
// Meff
    ...
    hmt2[ch][jetBin]                   ->Fill(TMath::Min(double(MT2(p_met.first, p_met.second, lt_p4, ll_p4, 0.0, false)), 100.), weight);	
    if (v_jetsNoEtaCut_p4.size() > 1) hmt2J[ch][jetBin]                ->Fill(TMath::Min(double(MT2J(p_met.first, p_met.second, lt_p4, ll_p4, v_jetsNoEtaCut_p4, 0.0, BISECT, false)), 300.),weight);	
  }
