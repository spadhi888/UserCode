...
void browseStacks( bool makePictures=false, bool wait=true , string referencePrefix="ttdil", TString outfilename = "stacks.ps",int addHistName = 1, Double_t maxYScaleF = 1., 
  ...
  */
  ...
  TObjArray* myNames = getMyHistosNames(referencePrefix.c_str(),"ee",keep2D);
  
  bool haveRefHistograms = gROOT->FindObjectAny(("ref_" + referencePrefix + "_"+  myNames->At(0)->GetName() + "_ee").c_str()) != 0;
  ...
  if (makePictures) c->Print(("out/" + outfilename + ".ps[").Data());
  for (int i=0; i<myNames->GetEntries(); i++) {
    ...
    for (int sample=0; sample<4; sample++) {
      ...
      hist::stack(Form("st_%s_%s",myNames->At(i)->GetName(),suffix[sample]),
		  Form("^[^_]+_%s_%s$",myNames->At(i)->GetName(), suffix[sample]), false, "",orderScheme, bsmName,haveRefHistograms, "data");
      THStack* thisStack = (THStack*) gROOT->FindObjectAny(Form("st_%s_%s", myNames->At(i)->GetName(), suffix[sample]));
      TH1F* hdata = (TH1F*)gROOT->FindObjectAny(Form("data_%s_%s", myNames->At(i)->GetName(), suffix[sample]));
	...
	refStack = (THStack*) gROOT->FindObjectAny(
      ...
      if(hdata != NULL){
	if(hdata->GetMaximum() > thisMaximum)
	  thisMaximum = maxYScaleF*hdata->GetMaximum();
	hdata->SetMaximum(thisMaximum);
      }
	...
	thisLeg->AddEntry(hdata, "data", "P");
      ...
      if(hdata != NULL){
	if(hdata->GetMaximum() > stackMax)
	  stackMax = hdata->GetMaximum();
	if(hdata->GetMinimum() < stackMin)
	  stackMin = hdata->GetMinimum();
      }
	...
	thisStack->Draw("hist");
	string xtitle( ((TH1*)gROOT->FindObjectAny((referencePrefix + Form("_%s_%s", myNames->At(i)->GetName(), suffix[sample])).c_str()))->GetXaxis()->GetTitle());
	string ytitle( ((TH1*)gROOT->FindObjectAny((referencePrefix + Form("_%s_%s", myNames->At(i)->GetName(), suffix[sample])).c_str()))->GetYaxis()->GetTitle());
	...
	if(hname.Contains("hnJet")) {
	  ...
	  if(hdata != NULL) {
	    hdata->GetXaxis()->SetLabelSize(0.075);
	    hdata->GetYaxis()->SetLabelSize(0.05);
	    hdata->GetXaxis()->SetTitle("#font[12]{N}_{jets}");
    ...
    if (makePictures) {
      ...
      c->Print(("out/" + outfilename + ".ps").Data());
