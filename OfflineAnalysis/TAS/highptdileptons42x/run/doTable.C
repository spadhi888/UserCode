...
//  gROOT->ProcessLine(".x Tools/setup.C");
  ...
  pair<Float_t, Double_t> stop_mm = doErrors(ftt, "stop_hnJet_mm");
  pair<Float_t, Double_t> stop_ee = doErrors(ftt, "stop_hnJet_ee");
  pair<Float_t, Double_t> stop_em = doErrors(ftt, "stop_hnJet_em");
  pair<Float_t, Double_t> stop_all = doErrors(ftt, "stop_hnJet_all");


  pair<Float_t, Double_t> ttop_mm = doErrors(ftt, "ttop_hnJet_mm");
  pair<Float_t, Double_t> ttop_ee = doErrors(ftt, "ttop_hnJet_ee");
  pair<Float_t, Double_t> ttop_em = doErrors(ftt, "ttop_hnJet_em");
  pair<Float_t, Double_t> ttop_all = doErrors(ftt, "ttop_hnJet_all");
...
//  float QCDE_all = sqrt(pow(qcd_all.second,2)+pow(qcd1_all.second,2));
  ...
  float total_ee = tt_ee.first + tw_ee.first + stop_ee.first + ttop_ee.first + wj_ee.first + DY_ee + ww_ee.first + wz_ee.first + zz_ee.first;
  float totalE_ee = sqrt(pow(tt_ee.second,2)+ pow(tw_ee.second,2) + pow(stop_ee.second,2) + pow(ttop_ee.second,2) + pow(wj_ee.second,2) + pow(DY_eeErr,2) + pow(ww_ee.second,2) + pow(wz_ee.second,2) + pow(zz_ee.second,2));

  float total_mm = tt_mm.first + tw_mm.first + stop_mm.first + ttop_mm.first + wj_mm.first + DY_mm + ww_mm.first + wz_mm.first + zz_mm.first;
  float totalE_mm = sqrt(pow(tt_mm.second,2)+ pow(tw_mm.second,2) + pow(stop_mm.second,2) + pow(ttop_mm.second,2) + pow(wj_mm.second,2) + pow(DY_mmErr,2) + pow(ww_mm.second,2) + pow(wz_mm.second,2) + pow(zz_mm.second,2));

  float total_em = tt_em.first + tw_em.first + stop_em.first + ttop_em.first + wj_em.first + DY_em + ww_em.first + wz_em.first + zz_em.first;
  float totalE_em = sqrt(pow(tt_em.second,2)+ pow(tw_em.second,2) + pow(stop_em.second,2) + pow(ttop_em.second,2) + pow(wj_em.second,2) + pow(DY_emErr,2) + pow(ww_em.second,2) + pow(wz_em.second,2) + pow(zz_em.second,2));
  
  float total_all = tt_all.first + tw_all.first + stop_all.first + ttop_all.first + wj_all.first + DY_all + ww_all.first + wz_all.first + zz_all.first;
  float totalE_all = sqrt(pow(tt_all.second,2)+ pow(tw_all.second,2) + pow(stop_all.second,2) + pow(ttop_all.second,2) + pow(wj_all.second,2) + pow(DY_allErr,2) + pow(ww_all.second,2) + pow(wz_all.second,2) + pow(zz_all.second,2));
  ...
  cout << "| SS Leptons | Total MC | ttbar | tW | s-top | t-top | Wjets | DY | WW | WZ | ZZ | Vgamma (not in total) |" <<  endl;
  cout.setf(ios::fixed, ios::floatfield);
  cout.precision(2);
  cout << "| ee | " << total_ee << pm << totalE_ee << " | "
       << tt_ee.first << pm << tt_ee.second << " | " 
       << tw_ee.first << pm << tw_ee.second << " | " 
       << stop_ee.first << pm << stop_ee.second << " | " 
       << ttop_ee.first << pm << ttop_ee.second << " | " 
...
//       << wc_ee.first << pm << wc_ee.second << " | " 
  ...
  cout << "| &mu;&mu; | "  << total_mm << pm << totalE_mm << " | "
       ...
       << stop_mm.first << pm << stop_mm.second << " | "
       << ttop_mm.first << pm << ttop_mm.second << " | "
...
//       << wc_mm.first << pm << wc_mm.second << " | "
  ...
  cout << "| e&mu; | "  << total_em << pm << totalE_em << " | "
       ...
       << stop_em.first << pm << stop_em.second << " | "
       << ttop_em.first << pm << ttop_em.second << " | "
...
//       << wc_em.first << pm << wc_em.second << " | "
  ...
  cout << "| total | "  << total_all << pm << totalE_all << " | "
       ...
       << stop_all.first << pm << stop_all.second << " | "
       << ttop_all.first << pm << ttop_all.second << " | "
