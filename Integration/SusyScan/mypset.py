...
# Define parameters
process.susyScanMassGluino = cms.EDProducer("DoubleValueProducer",
    value = cms.untracked.double(0)
)
process.susyScanMassSquark = cms.EDProducer("DoubleValueProducer",
    value = cms.untracked.double(0)
)
process.susyScanChi0 = cms.EDProducer("DoubleValueProducer",
    value = cms.untracked.double(0)
)
process.susyScanCrossSection = cms.EDProducer("DoubleValueProducer",
    value = cms.untracked.double(0)
)
process.susyScanRun = cms.EDProducer("DoubleValueProducer",
    value = cms.untracked.double(0)
)
...
process.ProductionFilterSequence = cms.Sequence(process.susyScanMassGluino+process.susyScanMassSquark+process.susyScanChi0+process.susyScanCrossSection+process.susyScanRun+process.generator)
...
def customise(process):
    ...
    MASSSQARK = 1040
    MASSCHI0 = 50
    SLHA = 'data/gmsbDiphoton_480_1040_50.slha'
    CROSSSECTION = 1.4360
    COMENERGY = 7000.
    RUN = 480104050

    process.source.firstRun = cms.untracked.uint32(10173)
    process.source.firstLuminosityBlock = cms.untracked.uint32(3967)
    process.generator.crossSection = cms.untracked.double(CROSSSECTION)
    process.generator.comEnergy = cms.double(COMENERGY)
    process.generator.PythiaParameters.SLHAParameters = cms.vstring('SLHAFILE = %s' % SLHA)

    process.susyScanMassGluino.value = cms.untracked.double(MASSGLUINO)
    process.susyScanMassSquark.value = cms.untracked.double(MASSSQARK)
    process.susyScanChi0.value = cms.untracked.double(MASSCHI0)
    process.susyScanCrossSection.value = cms.untracked.double(CROSSSECTION)
    process.susyScanRun.value = cms.untracked.double(RUN)

# need to add this later by hand
    process.RECOSIMoutput.outputCommands.extend (cms.untracked.vstring ('keep *_susyScan*_*_*'))
