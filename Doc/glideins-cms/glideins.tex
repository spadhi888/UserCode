...
\begin{figure}
\begin{center}
\includegraphics[scale=0.55]{glideinWMS_arch}
\end{center}
...
\end{itemize}
The late-binding WMS used by CMS is called \textbf{glideinWMS}. GlideinWMS is based on the Condor \cite{bib:condor} batch system, 
...
\vspace{2mm}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In principle, the problem of interoperability between different grids is reduced to having a Condor-G \cite{bib:condorg} client
...
between EGEE \cite{bib:egee} and OSG \cite{bib:osg} inside the CRAB \cite{bib:cms_crab} or ProdAgent layers of the software stack, thus requiring little special 
...
in the first ever use of NorduGrid \cite{bib:ng} resources for data analysis within CRAB using glideinWMS. The other grid flavors like 
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.54]{DataReprocess}
\end{center}
\caption{Overview of CMS data re-processing and resource usage statistics from the dashboard monitoring tool using glideinWMS.}
...
Fig.~\ref{fig:glideinT1stat}b) shows the number of simultaneous glide-ins running at various Tier-1 centers during the last week of March 2009 (See also Fig.~\ref{fig:merged_events}a) on simultaneously running glide-ins at one of the CMS Tier-1 sites).
...
uses CRAB, in order to perform user analyses at the Tier-2 centers.
...
the location of the input dataset via the CMS-specific Dataset Bookkeeping System (DBS) \cite{bib:cms_computing_arch}. Furthermore, it performs various checks along with 
...
log files (stdout/stderr). The overall process also updates the CMS Dashboard \cite {bib:dashboard} using MonALISA as the transport agent for user job monitoring.
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.55]{crabserver}
\end{center}
...
The pilot statistics and its client usage have been discussed in earlier sections.
...
The Computing Resource Execution And Management (CREAM) \cite{bib:cream} is a next generation web service interface 
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.55]{cms_cream}
\end{center}
...
\bibitem{bib:condorg}
Condor-g homepage, http://www.cs.wisc.edu/condor/condorg/
\bibitem{bib:egee}
...
\bibitem{bib:ng}
Nordic Data Grid Facility (NDGF) Web Page, http://www.nordugrid.org/
...
\bibitem{bib:dashboard}
J. Andreeva et al., ``Experiment Dashboard: the monitoring system for the LHC experiments'', In Proceedings of the 2007 Workshop on Grid Monitoring, Monterey, California, USA, June 2007. 
...
\bibitem{bib:cream}
P. Andreetto et al., ``CREAM: A simple, Grid-accessible, Job Management System for local Computational Resources'', Proc. XV International Conference on Computing in High Energy and Nuclear Physics (CHEP'06), Feb 13-17, 2006, Mumbay, India.
