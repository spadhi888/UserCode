...
5 events observed and 4.5 $\pm$ 1.7 expected from background.
...
a limit at 95\% CL of 7.1 events using the CL$_{\rm S}$ method.
The expected limit is 6.4 events.
% $7.8^{+3.6}_{-3.1}$ events.
In the MC we find $Acc \times Eff \times BR = 0.00233$, independent of $Z'$ mass. 
This results in an upper limit on the cross-section of 0.61 pb.
The limit includes uncertainty
on JES (12\%), $b$-tagging (10\%), lepton efficiencies (11\%), luminosity (2.2\%),
and PDF (3\%).
Our limit is a factor of 27 more stringent than our
2010 result\cite{sstop} and also a factor of 6 better
...
the best expected limit amongst regions SR1-7.  
...
For each point in parameter space we use the signal region that gives
the best expected limit amongst regions SR1-7, see Figure~\ref{fig:t1tttoptimize}.
...
For each point in parameter space we use the signal region that gives
the best expected limit amongst regions SR1-7, see Figure~\ref{fig:sbottomoptimize}.
The limits include all experimental 
uncertainties.   The JES and $b$-tag uncertainties are calculated point-by-point.
