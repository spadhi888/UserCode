...
the first (second) number is the $H_T$ (\met) threshold in GeV.
...
the best expected limit.  Limits are calculated using all experimental
...
Using the 
NLO$+$NLL cross-section for gluino pair production, we als place a limit
...
For each point in parameter space we use the signal region that gives
the best expected limit.  The limits include all experimental 
uncertainties.   The JES and btag uncertainties are calculated point-by-point.
