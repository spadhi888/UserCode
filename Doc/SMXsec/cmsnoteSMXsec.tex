\documentclass{cmspaper_pdf}
\usepackage{graphicx}

\begin{document}

%==============================================================================
% title page for few authors

\begin{titlepage}

  % select one of the following and type in the proper number:
  \cmsnote{2010/XXX}
  % \analysisnote{2005/000}
  % \internalnote{2007/007}
  % \conferencereport{2005/000}
  \date{21 June 2010}

  \title{Higher Order Standard Model cross sections at 7 TeV}

  \begin{Authlist}
%    The CMS Collaboration
    Guillelmo Gomez Ceballos Retuerto 
    \Instfoot{mit}{Massachusetts Inst. of Technology}
    Roberto Chierici
    \Instfoot{cnrs}{Institut de Physique Nucl\'eaire de Lyon}
    Fabio Cossutti 
    \Instfoot{infn}{Sezione di Trieste, INFN}
    Sanjay Padhi
    \Instfoot{ucsd}{University of California, San Diego}
    Fabian Stoeckli
    \Instfoot{cern}{CERN}
    Silvano Tosi
    \Instfoot{ipn}{Institut de Physique Nucl\'eaire de Lyon}

--------- Anyone I Missed from the list ----------

  \end{Authlist}

  \author{\bf(On behalf of the CMS Collaboration)}

  \begin{abstract}
    This note provides a compilation of the higher-order Standard Model cross sections using the latest available calculations for 
proton-proton collisions with 7 TeV centre-of-mass energy. The cross sections are based on a given choice of scale 
and parton distribution functions (PDFs) widely used in the CMS Collaboration for Monte Carlo simulations. 
The scale uncertainties and PDF uncertainties are provided for these choices. Cross sections using 
other higher order PDFs are also outlined along with their uncertanities.
  \end{abstract}

\end{titlepage}

\setcounter{page}{2}%

%==============================================================================
%
\input{intro.tex}
\input{kfactors.tex}
\input{assumptions.tex}
\input{highorder.tex}
\input{summary.tex}

\begin{thebibliography}{9}

\bibitem{mcfm}http://mcfm.fnal.gov/
\bibitem{pdg}http://pdg.lbl.gov/
\bibitem{fewz}http://www.hep.wisc.edu/~frankjp/FEWZ.html
\bibitem{cteq6m} J. Pumplin, D.R. Stump, J. Huston, H.L. Lai, Pavel M. Nadolsky and W.K. Tung, JHEP 0207:012,2002. 
\bibitem{Pythia} \textbf{PYTHIA}: T. Sjöstrand, S. Ask, R. Corke, S. Mrenna, P. Skands, http://home.thep.lu.se/~torbjorn/Pythia.html
\bibitem{Herwig} \textbf{HERWIG}: G. Corcella, I.G. Knowles, G. Marchesini, S. Moretti, K. Odagiri, P. Richardson, M.H. Seymour and B.R. Webber, JHEP 0101 (2001) 010
\bibitem{nnllttbar} Nikolaos Kidonakis, arXiv:0909.0037
\bibitem{nnllschannel} Nikolaos Kidonakis, arXiv:1001.5034
 
\end{thebibliography}

\end{document}
