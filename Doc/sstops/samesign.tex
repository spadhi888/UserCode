...
from simulation.  It is found to be $0.95 \pm 0.13 \%$, independent of $Z'$ 
mass (the uncertainty on this quantity is discussed below).  This acceptance
is calculated using the MadGraph {\tt DECAY} package; when Pythia is used
to decay the top quarks, the acceptance is 20\% higher, see
Fig.~\ref{fig:topdecays}.

\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.5\linewidth]{figs/effAccBRdecays.pdf}
\caption{The product of acceptance, efficiency, and branching ratio 
as determined
from MC as a function of $Z'$ mass.  
The red points correspond to events where the
top decays was handled by Pythia; the blue points correspond to 
events where the
top decay was handled by Madgraph, so that spin effects are properly 
accounted for.
In this last case the acceptance was corrected for the fact that the 
$W \to \ell \nu$ branching ratio in MadGraph is 11.1\% instead of 10.8\%.
\label{fig:topdecays}}
\end{center}
\end{figure}
...
This corresponds to a 95\% CL. upper limit for $\sigma(pp \to tt)$ of 17.0 pb.
...
would then correspond to $\frac{C_{RR}}{\Lambda^2} < 2.6$ TeV$^{-2}$ at 
