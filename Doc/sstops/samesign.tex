...
This final state is exactly the final state that one would expect from top-top production with both top quarks decaying as $t\rightarrow Wb$, $W\rightarrow \ell \nu$.
In AN 2010/247 we presented event yields and background expectations for several event selections.  
One of those event selections is very similar to that of the $t\bar{t}$ (opposite sign) dilepton cross-section analysis \cite{topxsection}, 
...
The results of the search in this kinematical region are summarized in Table 6 of AN 2010/247 v6 \cite{ssnote1}, 
...
The observed and expected SM event yields in 35~pb$^{-1}$ reproduced from Table 6 of AN 2010/247 v6 \cite{ssnote1}
are shown in Table \ref{tab:ssyields}.
The data-driven background prediction is based on a combination of estimating ``fake leptons''~\cite{fakenote} (FakeRate) 
and electrons reconstructed with the wrong sign~\cite{ssnote1} (Charge FlipRate). 
The probability for muons to be reconstructed with the wrong sign at the relevant momenta is negligible.
...
of SM samples \cite{ssnote1}. The data yield is in good agreement with the predication from both MC as 
...
Although the observed events have two positively charged di-leptons, we do not consider this a significant deviation from the prediction.

\subsection{Systematic Uncertainties}
\label{sec:sssystematics}

Details of the systematic uncertainties studied are discussed elsewhere~\cite{ssnote1}. 
We summarize the most important aspects for the previously described signal in Table \ref{tab:systSumm}.
...
only excludes up to $m_Z' \sim 510$ GeV for higher values of $f_R$.
